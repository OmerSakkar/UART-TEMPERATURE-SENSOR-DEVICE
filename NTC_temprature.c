...
float calculate_temperature(uint16_t adc_result	)
{
	...
	temprature -= 273; //Convert temp from kelvin to C.
...
{
	 static uint8_t oldest_sample = 0; //tracks the oldest sample index to overwrite it with new samples.
	 ...
	 {
		 moving_average += temprature_buffre[i]; //store the sum of the last AVERAGE_COUNT sample in the moving average variable
	 }
	 moving_average /= AVERAGE_COUNT; //find the average
